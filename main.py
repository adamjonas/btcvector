...
from src.utils import get_clean_json, connect_to_pinecone_index
...
log = setup_logger(out_file=f"{LOG_PATH}/es_to_pinecone_{date}.log", stderr_level=logging.INFO)


def start_data_conversion():
    try:
        pinecone_index = connect_to_pinecone_index()
        total_inserted_data_count = pinecone_index.describe_index_stats()["total_vector_count"]
        if total_inserted_data_count == 0:
            ...
            # Next time we will just insert the data into pinecone, not extract from ES.
            json_data_path = os.path.join(f"{DATA_PATH}", f"{ES_INDEX}_refined.json")
            pinecone_data_insertion(json_data_path, pinecone_index, start_from=total_inserted_data_count)
            log.info("PROCESS OF DATA INSERTION COMPLETED")
